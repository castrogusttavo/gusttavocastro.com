...
function Post(props) {
  ...
  const url = `https://castrogusttavo.com/${props.slug}`
  const date = new Date(props.date).toISOString()
  const image = props.image
    ? `https://castrogusttavo.com${props.image}`
    : 'https://castrogusttavo.com/static/images/home-opt.jpg'
...
export async function getStaticProps({ params }) {
  try {
    ...
    const base = isProd ? 'https://castrogusttavo.com' : 'http://localhost:3000'
